  ...
  <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
    ...
    <div class="flex items-start justify-between mb-6">
      <div class="flex-1">
        <h2 class="text-2xl font-bold text-gray-900">
          {{ store.instance.description || 'MYPM Instance' }}
        </h2>
    ...
    <div class="mb-6">
      <div class="flex items-center justify-between mb-3">
        <h3 class="text-lg font-semibold text-gray-900">Email Keys (Domains)</h3>
        <button
      ...
      <!-- Add Key Form -->
      <div v-if="showAddKey" class="bg-gray-50 rounded-lg p-4 mb-4">
        <form @submit.prevent="handleAddKey" class="flex gap-2">
          <input
            v-model="newKey"
            type="text"
      ...
      <!-- Keys List -->
      <div v-if="store.instanceKeys.length > 0" class="space-y-2">
        <div
      ...
      <div v-else class="bg-yellow-50 border-l-4 border-yellow-400 rounded-lg p-4">
        <div class="flex items-start">
          <svg class="w-6 h-6 text-yellow-600 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
          <div class="flex-1">
            <h4 class="text-sm font-semibold text-yellow-800 mb-1">No keys added yet!</h4>
    ...
    <div class="bg-gray-50 rounded-lg p-4">
      <div class="text-sm text-gray-500 mb-1">Secret (API Key)</div>
...
import { EMAIL_DOMAIN, keyAddress } from '../config';
